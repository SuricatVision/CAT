...
class BaseOptions:
    ...
    def initialize(self, parser):
        ...
        parser.add_argument(
            ...
            action='store_true',
            help=
            'whether you will load the data into the memory to bypass the IO.')
        ...
        parser.add_argument(
            '--drn_path',
            type=str,
            default='drn-d-105_ms_cityscapes.pth',
        ...
        parser.add_argument(
            ...
            help=
            'the original cityscapes dataset path (not the pix2pix preprocessed one)'
        )
        parser.add_argument(
            '--table_path',
            type=str,
            default='datasets/table.txt',
            help=
            'the path to the mapping table (generated by datasets/prepare_cityscapes_dataset.py)'
        )
        parser.add_argument('--padding_type',
                            type=str,
                            default='reflect',
                            choices=['reflect', 'zero'],
                            help='padding type to use [reflect | zero]')
        return parser
    ...
    def parse(self, verbose=True):
        ...
        if hasattr(opt, 'contain_dontcare_label') and hasattr(
                opt, 'no_instance'):
            opt.semantic_nc = opt.input_nc + (1 if opt.contain_dontcare_label
                                              else 0) + (0 if opt.no_instance
                                                         else 1)
