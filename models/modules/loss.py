...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real, for_discriminator=True):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            if isinstance(prediction, list):
                ...
                for pred_i in prediction:
                    if isinstance(pred_i, list):
                        pred_i = pred_i[-1]
                    loss_tensor = self(pred_i, target_is_real,
                                       for_discriminator)
                    bs = 1 if len(
                        loss_tensor.size()) == 0 else loss_tensor.size(0)
                ...
                if for_discriminator:
                    if target_is_real:
                        minval = torch.min(prediction - 1,
                                           self.get_zero_tensor(prediction))
            ...
            raise NotImplementedError('gan mode %s not implemented' %
                                      self.gan_mode)
        return loss
